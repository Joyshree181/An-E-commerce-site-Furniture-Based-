					...
					<table class="ui celled unstackable table">
						...
						if ($result = $conn->query($query)) {
							...
							while ($row = $result->fetch_assoc()){
								printf("<tr>");
								printf("<td>%s</td> <td>%s</td>  <td>%s</td>  <td>%s</td>   <td> <a href='db/delete.php?id=%s'>Delete</a> <a href='edit_form.php?id=%s'>Edit</a> </td>", $ser, $row["pay_id"], $row["Title"], $row["address"], $row["pay_by"],$row["ID"], $row["ID"]);
								printf("</tr>");
								$ser++;
							}
						}
